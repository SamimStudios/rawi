...
const addressError = !address ? `Failed to build address for ${fieldRef}` : null;
  ...
  const effectiveValue = useMemo(() => {
    return draftValue !== undefined ? draftValue :
           (dbValue !== undefined ? dbValue : fieldDefinition?.default_value);
  ...
  const help = fieldDefinition.ui?.help?.fallback || fieldDefinition.ui?.help?.key || '';
  const rules = fieldDefinition.rules || {};

  const [internalError, setInternalError] = useState<string | null>(null);
  const errorMsg = internalError || null;
  ...
  const handleChange = useCallback(async (val: any) => {
    setValue(val);
    validateValue(val);
  }, [setValue, validateValue]);

  // In idle mode, show label:value compactly
  if (isReadOnly) {
  ...
  const renderEdit = () => {
    switch (widget) {
      ...
      case 'select': {
        ...
        return (
          <Select value={currentValue || ''} onValueChange={handleChange}>
            <SelectTrigger className={cn(errorMsg && "border-destructive")}>
      ...
      case 'radio': {
        ...
        return (
          <RadioGroup
            ...
            className={cn(errorMsg && "border border-destructive rounded-md p-2")}
          >
            {options.map((opt: any, i: number) => (
      ...
      case 'checkbox':
        return (
          <div className={cn("flex items-center space-x-2", errorMsg && "text-destructive")}>
            <Checkbox
      ...
      case 'date':
        return (
          <Input type="date" value={currentValue || ''} onChange={(e) => handleChange(e.target.value)} className={cn(errorMsg && "border-destructive")} />
        );

      case 'time':
        return (
          <Input type="time" value={currentValue || ''} onChange={(e) => handleChange(e.target.value)} className={cn(errorMsg && "border-destructive")} />
        );

      case 'email':
        return (
          <Input type="email" value={currentValue || ''} onChange={(e) => handleChange(e.target.value)} placeholder={placeholder} className={cn(errorMsg && "border-destructive")} />
        );

      case 'url':
        return (
          <Input type="url" value={currentValue || ''} onChange={(e) => handleChange(e.target.value)} placeholder={placeholder} className={cn(errorMsg && "border-destructive")} />
        );

      case 'number':
        return (
          <Input type="number" value={currentValue || ''} onChange={(e) => handleChange(e.target.value ? Number(e.target.value) : '')} placeholder={placeholder} min={rules?.min} max={rules?.max} className={cn(errorMsg && "border-destructive")} />
        );
      ...
      case 'file':
        return (
          <Input type="file" onChange={(e) => {
            ...
            if (file) setValue({ name: file.name, size: file.size, type: file.type });
          }} className={cn(errorMsg && "border-destructive")} />
        );
